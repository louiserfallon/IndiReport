...
#take data only from the top 4 retailers
data_ind_summary_pre <- fulldata[fulldata$shop_desc %in% c("1TESCO","2ASDA","3SAINSBURYS","4MORRISONS") &
...
tmpprods <- head(tmp$Var1,50)

df_ind_summary$product[!df_ind_summary$product %in% tmpprods] <- "Other"

```

The data is made up of around 29,000 UK households' purchases of canned and bottled carbonates and soft drinks over a year. The following analysis uses only data from the top 4 retailers, as more reliable data is available on product availability for these stores. The full data is made up of varying sized packages of beverages, including, for example, 330ml cans and 2 litre bottles. The analysis is made meaningful by focusing only on canned drinks, to focus on true competitors. Own brand products are treated as one product, and low volume brands are grouped together to narrow the data to 40 products. The price range for these products can be seen in the figure below, energy drinks such as Red Bull have a higher price than average, and interestingly, it does not seem like own brand products have a lower price than average, which may be due to the promotional pricing often used by major brands^[Citation needed]:
...
The key product level explanatory variables are price and promotion, which are supplemented by household level variables calculated from the data. The dataset did not already contain household-specific variables such as income or number of children, to be used as additional explanatory variables in the model, despite the fact they are often included as part of a UPC dataset. However, as there was a large enough time window, it was possible to calculate household-level variables from an initialisation period. In this case a loyalty variable was calculated: the proportion spent on each brand in the first month. An average spend variable was also calculated: the average vaule spent in the category per week over the first month. These variables account for some of the heterogeneity across households.
...
findfunction <- function(x, date, shop) {
  if (sum(df_ind_summary$product == x["Var1"] &
                     ...
                     df_ind_summary$shop == shop) > 0) {return(max(df_ind_summary$priceper330ml[
...
The multinomial logit model fit above is focusing on brand choice, and ignores other key business questions such as: what are the drivers for how often consumers come to the shop, and for how much they buy when they are there. For this, a . The elasticity that is produced by this model is total purchase elasticity, and this can be added to the brand choice elasticity from the multinomial logit model to get the overall elasticity. (reference xxxx)
